...
import { trendLimiter, enforceLimit } from "@/app/api/_lib/rate-limit";
...
import { enqueueTrend } from "@/lib/queue/trend-queue";
...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  return handle(async () => {
    ...
    if (!brief) throw new NotFoundError("No trend brief found for this client");
    return ok({ brief });
  });
}
...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  return handle(async () => {
    ...
    const auth = requireRequestAuth();
    await enforceLimit(trendLimiter, `trend:${auth.ctx.agencyId}:${params.id}`);
    ...
    const jobId = await enqueueTrend({
